    ...
    public class PilotoController {
        ...
        @DeleteMapping("/eliminar/{dorsal}")
        public ModelAndView eliminarPiloto(@PathVariable Integer dorsal, HttpServletRequest request) {
            logger.debug("Entrando a eliminarPiloto con dorsal: {}", dorsal);
            ...
            try {
                ...
                if (user.getRol() != Rol.ADMIN && user.getRol() != Rol.JEFE_DE_EQUIPO) {
                    logger.debug("El usuario no tiene permisos para eliminar un piloto.");
                    mv.setViewName("error");
                    mv.addObject("mensajeError", "No tienes permiso para eliminar un piloto.");
                    return mv;
                }

                Piloto piloto = pilotoService.obtenerPilotoPorDorsal(dorsal).orElseThrow(() -> new NoSuchElementException("Piloto no encontrado"));

                // Verificar si el piloto está en una encuesta activa
                if (encuestaService.estaPilotoEnEncuestaActiva(piloto.getDorsal())) {
                    mv.setViewName("error");
                    mv.addObject("mensajeError", "El piloto está en una encuesta activa y no puede ser eliminado.");
                    return mv;
                }

                pilotoService.eliminarPiloto(dorsal);
                mv.addObject("successMessage", "El piloto ha sido eliminado exitosamente.");
                ...
                logger.error("Error al eliminar el piloto con dorsal: {}", dorsal, e);
                mv.setViewName("error");
                mv.addObject("mensajeError", "Error al eliminar el piloto.");
            }
            return mv;
        }
